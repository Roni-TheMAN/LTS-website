...
function RiskTooltip({
                         enabled,
                         children,
                         message = "⚠️ Do this at your own risk.\nDuplicate Stripe payments are possible.",
                     }: {
    enabled: boolean;
    ...
    const updatePos = React.useCallback(() => {
        ...
        const tooltipWidth = 260; // keep in sync with max-w below
        const tooltipHeight = 64; // approx, fine for clamping

        // prefer above the button; fallback below if too close to top
        const preferredTop = r.top - 10; // we'll subtract tooltip height later
        const aboveTop = preferredTop - tooltipHeight;
        const belowTop = r.bottom + 10;

        const top = aboveTop < 8 ? belowTop : preferredTop - tooltipHeight;

        const left = clamp(r.left + r.width / 2 - tooltipWidth / 2, 8, window.innerWidth - tooltipWidth - 8);
        const clampedTop = clamp(top, 8, window.innerHeight - tooltipHeight - 8);
        ...
        >
            ...
            {enabled && open && pos
                ? createPortal(
                    ...
                    >
                        <div className="bg-red-600 text-white text-xs font-medium px-3 py-2 rounded-md shadow-lg max-w-[260px] whitespace-pre-line text-center">
                            {message}
                        </div>
                    </div>,
                    document.body
...
const OrderDetail: React.FC = () => {
    ...
    const canCollectPayment =
        !!order && order.payment_status !== "paid" && items.length > 0 && (order.total_cents ?? 0) > 0;
            ...
            <span className="hover:text-slate-900 cursor-pointer" onClick={() => navigate("/orders")}>
              Orders
                ...
                <div className="flex items-center gap-3">
                    ...
                    <RiskTooltip enabled={canCollectPayment}>
                        <button
                            className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-sm transition-colors border ${
                                canCollectPayment
                                    ? "bg-slate-900 hover:bg-slate-800 text-white border-slate-900"
                            ...
                            onClick={() => {
                                ...
                                navigate(`/orders/${order.id}/checkout`);
                            }}
